...
func main() {
	...
	goat.PrintSuccess("This demo will walk you how Goat works.  To see how the things demonstrated here are done, take a look at the code in this file:")
	goat.PrintInfo(file)
...
func printRoot() {
	p := goat.Root()
	goat.PrintIndent("Root path from the demo main package: " + p)
...
func demoCWD() {
	c := goat.CWD()
	goat.PrintIndent("CWD from the demo main package: " + c)
...
func demoDatabase() {
	...
	goat.PrintSuccess("Since the demo credentials in our config file are just examples, goat will panic if we try and use them to connect.")
