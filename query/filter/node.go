...
func (n *node) WhereField(field string, op Operator, value interface{}) Filter {
	filter := NewCondition(field, op, value)
	return n.where(filter, logicAnd)
}

func (n *node) OrWhereField(field string, op Operator, value interface{}) Filter {
	filter := NewCondition(field, op, value)
	return n.where(filter, logicOr)
}

func (n *node) Apply() (string, []interface{}, error) {
