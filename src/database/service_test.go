...
func TestNewServiceGORM_MainDB(t *testing.T) {
	...
	s := NewServiceGORM(Config{
		MainConnectionConfig: config,
	})

	require.Len(t, s.connections, 1, "unexpected number of connections")

	result, ok := s.connections[dbMainConnectionKey]
...
func TestNewServiceGORM_GetCustomDB(t *testing.T) {
	...
	require.Len(t, s.connections, 2, "unexpected number of connections")

	result, ok := s.connections["custom_connection"]
