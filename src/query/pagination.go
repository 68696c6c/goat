...
func NewPaginationFromGin(cx *gin.Context) Pagination {
	result := Pagination{
		Page:       pageDefault,
	...
	page, err := strconv.ParseInt(cx.Query(pageKey), 10, 32)
	if err == nil {
		result.Page = page
	}

	pageSize, err := strconv.ParseInt(cx.Query(pageSizeKey), 10, 32)
	if err == nil {
...
func NewPaginationFromValues(page, size, total int64) Pagination {
	if total == 0 {
		println("PAGINATION FROM VALUES TOTAL IS ZERO")
	} else {
		println("PAGINATION FROM VALUES TOTAL IS OK")
	}
	return Pagination{
		Page:     page,
		PageSize: size,
